...
int binExp(int a,int b,int M){int res=1;while(b){if(b&1) res=((res%M)*(a%M))%M;a=((a%M)*(a%M))%M;b>>=1;}return res;}
...
int GCD(int a, int b, int n)
{
    if(a==b)
    {
        return (binExp(a,n,mod) + binExp(b,n,mod)) % mod;
    }
    ...
    for(int i=1; i*i<=num; i++)
    {
        if(num%i==0)
        {
            int temp = (binExp(a,n,i) + binExp(b,n,i)) % i;
            if(!temp) ans = max(ans, i);
            temp = (binExp(a,n,num/i) + binExp(b,n,num/i)) % (num/i);
            if(!temp) ans = max(ans, num/i);
        }
    }
    return ans % mod;
}
